...
class SparseImage:
    ...
    def clear_all(self):
        print("This will remove the patch_properties_dict and image_properties_dict.")
        print("It is useful you want to restart the analysis from scratch.")
        print("It will not modify the _spot_properties_dict.")
        self._patch_properties_dict = {}
        self._image_properties_dict = {}

    def pixel_to_raw(
        self, x_pixel: torch.Tensor, y_pixel: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """ Convert the pixel coordinates to the raw_coordinates"""
    ...
    def raw_to_pixel(
        ...
        """ Convert the raw_coordinates to the pixel_coordinates """
    ...
    def to_rgb(self,
        ...
        """
        Make an 3 channel RGB image

        Args:
            spot_size: size of sigma of gaussian kernel for remdering the spots
    ...
    def analyze_with_tiling(
        ...
        The predictions are stored in the dictionary patch_properties_dict.
        ...
        Args:
            ...
            batch_size: int, how many crops to process simultaneously (default = 64)
            n_patches_max: int, how many patched to use to classify the current picture (default = 100)
            store_crops: bool, if true the crops are stored in the patch_dictionary
                under the key :attr:'store_crops_key'.
            store_crops_key: str, the key associated with the crops. Only used if :attr:'store_crops' == True.
            overwrite: bool, in case of collision between the keys in self.patch_properties_dict and the dictionary
                returned by analyzer, this variable controls when to overwrite them.
        """
        ...
        assert len(feature_names) == len(patch_analyzers), "Error, {0} vs {1}".format(feature_names, patch_analyzers)
    ...
    def patch_property_to_image_property(
            self,
            keys_to_transfer: Union[str, List[str]],
            overwrite: bool = False,
            verbose: bool = False):
        """
        ...
        Args:
            keys_to_transfer: the key or keys of the quantity to transfer from patch_properties_dict to image_properties_dict.
